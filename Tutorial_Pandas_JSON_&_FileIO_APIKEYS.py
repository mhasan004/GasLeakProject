...
# 1) storing key safely to a file named config.py, which git will ignore
MY_API_KEY = "abc123"
with open("config.py", "w+") as f:
    f.write("""api_key = \"{}\"""".format(MY_API_KEY))

# 2) can delete the the code of 1). Run this to get keys
import config
c = some_API_function_that_needs_key(config.api_key....)
...
# GET ROW # where col == something (GET INDEX)
    deleteTheseRows.append( df.index[df['ColName'] == "SomeVal"].tolist()[0] ) #can go one row at a time and run this to get a list of rows to del that got this val in this col  
...
# IMPORT CSV TO PD AND RENAME COLS:
    df = pd.read_csv(datafile, names = ['cola', 'colb', 'colc'], skiprows = 1)

# GET RID OF ALL ROWS THAT HAS A NaN VALUE FOR A COL:
    df = df[df['colA'].isnull() == False]
...
# Merging two DF: https://pandas.pydata.org/pandas-docs/stable/user_guide/merging.html
    # NEED SAME KEY NAME:
    mergedDF = pd.merge(seasonalDF, demoDF, on=["GEOID"], how='left') 


    # Old way dont work well:
