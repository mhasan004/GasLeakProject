...
for row in range(0,len(monthlyDF)):
    ...
    map.set_title(label = 'Number of Gas Leak Reports per Census Tract for\n{0}\n(Showing {2} GeoIDs/Census Tracts)'.format(thisMonthStr, len(countyTract_list), len(thisMonthPlotGDF)), fontdict={'fontsize': 20}, loc='center')
    # if len(countyTractMonth_list) != 0 and len(thisMonthPlotGDF) != 0: #there is a month that has one tract but no geoid! so cant get the legend
    #     leg = map.get_legend()
